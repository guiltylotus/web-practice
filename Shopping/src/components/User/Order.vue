  ...
  <div class="hone-container">
    ...
    <div class="order-container">
      <div class="card">
        ...
        <div class="collapse show bag-item-collapse" v-for="(order, key) in orderAccount" :key="key" v-if="!order.paid" data-toggle="modal" :data-target="'#' + order.id">
          <div class="card-body">
            <div class="row">
    ...
    <div class="order-container">
      <div class="card">
        ...
        <div class="collapse show bag-item-collapse-paid" v-for="(order, key) in orderAccount" :key="key" v-if="order.paid" data-toggle="modal" :data-target="'#' + order.id">
          <div class="card-body">
            <div class="row">
              <div class="col-6 text-left">
                <strong>order no {{order.id}}</strong>
    ...
    <!-- Modal -->
    <div v-for="(order, key) in orderAccount" :key="key">
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <div class="text-left order-modal-footer">
                <h5> Total: {{order.totalCost}}<i>vnđ</i></h5>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async getOrderByAccount (account) {
      ...
      this.orderAccount = response
      for (let order in this.orderAccount) {
        var totalCost = 0
        var obj = this.orderAccount[order].items
        for (let item in obj) {
          if (obj[item].quantity !== 0) {
            totalCost += obj[item].quantity * obj[item].price
          }
        }
        this.orderAccount[order].totalCost = totalCost
      }
